...
const ModifyPassword: FC<Props> = ({visible, onClose}) => {
  ...
  return (
    <Dialog title='包台密码' visible={isShow} onClose={() => setIsShow(false)} onAnimationend={close}>
      <Column style={{width: '428px', padding: '0 42px'}}>
        <Input label="请输入旧密码" type='password' rules={rules} onValidate={() => console.log('奥错')}/>
      </Column>
      <Row style={{width: '325px', margin: '46px auto 18px', padding: '15px 10px'}} justify="center" ailgn="center">
